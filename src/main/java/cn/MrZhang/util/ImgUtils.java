...
public class ImgUtils {
    ...
    public static byte[] thumbBySizeWithScale(byte[] content, int width, int length) {
        ...
        try {
            Thumbnails.of(inputStream).size(width, length).toOutputStream(outputStream);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static byte[] thumbBySizeWithScale(String filePath, int width, int length) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try {
            Thumbnails.of(filePath).size(width, length).toOutputStream(outputStream);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static byte[] thumbBySizeWithoutScale(byte[] content, int width, int length) {
        ...
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try {
            Thumbnails.of(inputStream).size(width, length).keepAspectRatio(false).toOutputStream(outputStream);
        } catch (IOException e) {
            e.printStackTrace();
        }
